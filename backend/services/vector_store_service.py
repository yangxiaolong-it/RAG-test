...
class VectorStoreService:
    ...
    def _get_chroma_client(self) -> Client:
        """
        获取或初始化 Chroma 客户端
        """
        if self._chroma_client is None:
            self._chroma_client = Client(self._chroma_settings)
    ...
    def _index_to_chroma(self, embeddings_data: Dict[str, Any], config: VectorDBConfig) -> Dict[str, Any]:
        ...
        if not base_name:
            base_name = "doc"
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        collection_name = f"{base_name}_{embeddings_data.get('embedding_provider', 'unknown')}_{timestamp}"
        ...
        for idx, emb in enumerate(embeddings_data["embeddings"], start=1):
            ids.append(str(idx))
    ...
    def get_collection_info(self, provider: str, collection_name: str) -> Dict[str, Any]:
        ...
        if provider == VectorDBProvider.CHROMA:
            ...
            try:
                ...
                logger.error(f"Error getting Chroma collection info for {collection_name}: {str(e)}")
                return {
                    "name": collection_name,
                    "num_entities": 0,
                    "metadata": {}
